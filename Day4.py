...
for x in get_input():
    if x != "":
        tmp_line = x.split(" ")
        tmp_dict = {item.split(":")[0]:item.split(":")[1] for item in tmp_line}
...
print(len(all_passports), len(valid_passports))
#2
curr_passport = {}
all_passports = []
valid_passports = []
...
for x in get_input():
    if x != "":
        ...
        if all(field in curr_passport for field in req_keys):
            valid_keys = 0
            for key, value in curr_passport.items():
                if key == "byr":
                    ...
                    if re.match('^#[0-9a-f]{6}|$', value):
                        valid_keys+=1
                elif key == "ecl" and value in ecl_allowed:
                    valid_keys+=1
                elif key == "pid":
