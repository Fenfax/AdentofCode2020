...
def get_input():
    inp = '''LLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLL.LLLLLL.LLLLLLLLLLLLL.LLLLLLLL
...
LLLLLLLLLL.LLLLLLLLL.LL.LLLLL.LLLLLLLLLLLLL.LLLLLLL.LLLLLL.LLLLLLLL.LLLLLLLLLLLLLLL.LLLLLLLLLLLLL'''
    return [[x for x in out]for out in inp.split("\n")]
...
def cnt_occ_seat(x,y,grid):
    ...
    for x2 in range(-1,2,1):
        for y2 in range(-1,2,1):
            ...
            pos_x = x + x2
            pos_y = y + y2
            if 0 <= pos_x < len(grid) and 0 <= pos_y < len(grid[x]) and grid[pos_x][pos_y] == "#":
                cnt += 1
    return cnt 
def cnt_occ_seat2(x,y,grid):
    cnt = 0
    ...
    for x2 in range(-1,2,1):
        for y2 in range(-1,2,1):
            ...
            pos_x = x + x2
            pos_y = y + y2
            while 0 <= pos_x < len(grid) and 0 <= pos_y < len(grid[x]) and grid[pos_x][pos_y] == ".":
                # print(pos_x,pos_y)
                if x2 > 0:
                    pos_x += 1
                elif x2 < 0:
...
def s1(inp):    
    prev_run = []
    while prev_run != inp:
        prev_run = [x.copy() for x in inp]
        for c,x in enumerate(inp):
            for i,y in enumerate(x):
                occ_seat = cnt_occ_seat(c,i,prev_run)
                if y == "#" and occ_seat >= 4:
                    inp[c][i] = "L"
                if y == "L" and occ_seat == 0:
                    inp[c][i] = "#"
    print(len([x for y in inp for x in y if x == "#"]))
    #print(inp,cnt)
...
def s2(inp):    
    ...
    while inp != prev_run:                   
        ...
        for c,x in enumerate(inp):
            for i,y in enumerate(x):
                occ_seat = cnt_occ_seat2(c,i,prev_run)
                if y == "#" and occ_seat >= 5:
                    inp[c][i] = "L"
                if y == "L" and occ_seat == 0:
                    inp[c][i] = "#"
