...
for x in get_input():
  ...
  if len(char) > 0:
    minpos=x.index("-")
    min_char=x[0:minpos]
    max_char=x[minpos+1:pos-2]
    # print(char,min_char,max_char)
    #if int(min_char) <= x.count(char)-1 <= int(max_char):
    #  ok_cnt+=1
    print(x[pos+1:],int(max_char))
    if (x[pos+1:][int(min_char) ] == char and not x[pos+1:][int(max_char)] == char):
      ok_cnt+=1
    if (not x[pos+1:][int(min_char) ] == char and x[pos+1:][int(max_char)] == char):
      ok_cnt+=1
