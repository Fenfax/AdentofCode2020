...
def count_occurences(long_string):
    out_list = {}
    for x in long_string:
        cnt = 0
        if x in out_list.keys():
            continue
        else:
            cnt = len([char for char in long_string if char == x])
            out_list[x] = cnt
    return out_list
...
for group in groups:
    ...
    s1.extend(list(set(unique_ques)))
print(len(s1))

#2
total = 0
for group in groups:
    occ = count_occurences("".join(group))
    total +=len([x for x,y in occ.items() if y == len(group)])
print(total)
